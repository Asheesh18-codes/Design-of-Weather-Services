...
interface MetarResult {
  raw: string;
  parsed: ParsedMetar | { error: string };
  summary: string;
}

export function parseMetar(metar: string): MetarResult | null {
  ...
  try {
    ...
    if (cloudMatches) {
      cloudMatches.forEach((cloud: string) => {
        const cloudMatch = cloud.match(/(CLR|SKC|FEW|SCT|BKN|OVC)(\d{3})?/);
        if (cloudMatch) {
          const coverageKey = cloudMatch[1] as keyof typeof coverageMap;
          const coverage = coverageMap[coverageKey] || cloudMatch[1];
    ...
    const vis = parseFloat(metar.match(/(\d{1,2})SM/)?.[1] || "10") || 10;
    ...
    return {
      raw: metar,
      summary: "Unable to parse METAR - " + message,
...
interface TafResult {
  raw: string;
  parsed: { station: string; issueTime: string; validPeriod: string; periods: TafPeriod[] } | { error: string };
  summary: string;
}

export function parseTaf(taf: string): TafResult | null {
  if (!taf) return null;
  ...
  try {
    ...
    lines.forEach((line: string) => {
      ...
      if (cloudMatches) {
        cloudMatches.forEach((cloud: string) => {
          ...
          if (cloudMatch) {
            const coverageKey = cloudMatch[1] as keyof typeof tafCoverageMap;
            const coverage = tafCoverageMap[coverageKey];
    ...
    const tafCoverageMap: Record<string, string> = {
      'FEW': 'Few',
      'SCT': 'Scattered',
      'BKN': 'Broken',
      'OVC': 'Overcast'
    ...
    return {
      raw: taf,
      summary: "Unable to parse TAF - " + message,
      parsed: { error: "Parsing failed" }
    };
  }
}
...
interface NotamResult {
  raw: string;
  parsed: { id: string; airport: string; category: string; details: string; severity: string; effectiveDate: string; expiryDate: string; additionalInfo: string } | { error: string };
  summary: string;
}

export function parseNotam(notam: string): NotamResult | null {
  ...
  try {
    ...
    return {
      raw: notam,
      summary: "Unable to parse NOTAM - " + message,
      parsed: { error: "Parsing failed" }
    };
  }
}
...
interface PirepResult {
  raw: string;
  summary: string;
}

export function parsePirep(pirep: string): PirepResult {
  if (!pirep) return { raw: pirep || '', summary: 'No PIREP data provided.' };
  ...
  // Abbreviation maps
  const tbMap: Record<string, string> = { "NEG": "none", "LGT": "light", "MOD": "moderate", "SEV": "severe", "LGT-MOD": "light to moderate", "MOD-SEV": "moderate to severe", "EXTRM": "extreme", "CHOP": "choppy" };
  const icMap: Record<string, string> = { "NEG": "none", "LGT": "light", "MOD": "moderate", "SEV": "severe", "TR": "trace" };
  const skyMap: Record<string, string> = { "CLR": "clear", "BKN": "broken clouds", "OVC": "overcast", "SCT": "scattered clouds", "FEW": "few clouds", "SKC": "sky clear" };
  ...
  let skyDesc = sky;
  if (sky) {
    skyDesc = sky.replace(/([A-Z]{3})(\d{3})?-?(TOP)?(\d{3})?/g, (m: string, cov: string, base: string, top: string, topAlt: string) => {
...
interface SigmetResult {
  raw: string;
  summary: string;
}

export function parseSigmet(sigmet: string): SigmetResult {
  if (!sigmet) return { raw: sigmet || '', summary: 'No SIGMET data provided.' };
  ...
  detailsText = detailsText.replace(/BTN\s*FL(\d{3})\s*AND\s*FL(\d{3})/gi, (m: string, fl1: string, fl2: string) => `between ${parseInt(fl1,10)*100} and ${parseInt(fl2,10)*100} feet`);
  detailsText = detailsText.replace(/FL(\d{3})/gi, (m: string, fl: string) => `${parseInt(fl,10)*100} feet`);
